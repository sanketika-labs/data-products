...
import org.sunbird.lms.exhaust.collection.{AssessmentData, CollectionBatch, CourseData, Metrics, ProgressExhaustJobV3}
...
//    System.setProperty("sunbird.compositesearch.base.url", "http://localhost:9092")
  ...
  "ProgressExhaustReportV3" should "generate the report with all the correct data" in {
    ...
    val metrics:Metrics = ProgressExhaustJobV3.execute()
    ...
    val filePath = ProgressExhaustJobV3.getFilePath(batch1, requestId)
    val jobName = ProgressExhaustJobV3.jobName()
    ...
    val assessmentData = ProgressExhaustJobV3.filterAssessmentsFromHierarchy(List(), Map(), AssessmentData("do_1130928636168192001667", List()))
  ...
  it should "provide hierarchy data on module level" in {
    ...
    var hierarchyModuleData = ProgressExhaustJobV3.getCollectionAggWithModuleData(collectionBatch, hierarchyData).collectAsList().asScala
    ...
    val hierarchyModuleData1 = ProgressExhaustJobV3.getCollectionAggWithModuleData(collectionBatch, hierarchyData).collectAsList().asScala
    ...
    var parseHierarchyData = ProgressExhaustJobV3.logTime(ProgressExhaustJobV3.parseCourseHierarchy(data, 3, prevData, 2), "Execution of ParseCourseHierarchy method")
    ...
    hierarchyModuleData = ProgressExhaustJobV3.getCollectionAggWithModuleData(collectionBatch, hierarchyData).collectAsList().asScala
    hierarchyModuleData.map(f => f.getString(0)) should contain theSameElementsAs  List("user-001", "user-002", "user-003", "user-004")
    hierarchyModuleData.map(f => f.getString(1)) should contain allElementsOf List("do_1130928636168192001667")
    hierarchyModuleData.map(f => f.getString(2)) should contain allElementsOf List("batch-001")
    hierarchyModuleData.map(f => f.getInt(3)) should contain allElementsOf List(100)
  ...
  it should "validate the report path" in {
    ...
    val jobConfig = JSONUtils.deserialize[JobConfig](strConfig)
    implicit val config = jobConfig
    val onDemandModeFilepath = ProgressExhaustJobV3.getFilePath(batch1, requestId)
    val reportDate = getDate("yyyyMMdd").format(Calendar.getInstance().getTime())
    onDemandModeFilepath should be(s"progress-exhaust/$requestId/batch-001_progress_$reportDate")

    val standAloneModeFilePath = ProgressExhaustJobV3.getFilePath(batch1, "")
